...
def main_menu(role: Role | str | None = None) -> ReplyKeyboardMarkup:
    ...
    Коту показываем сменные инструменты: замены и шансы для обмена.
    Котику не показываем то, что ей недоступно, чтобы меню было проще.
    """
    ...
    if raw == Role.KOT.value:
        base_rows.append([KeyboardButton(text="🔁 Замены"), KeyboardButton(text="🔎 Шансы для обмена")])
    else:
        # У Котика ненормированный график, поэтому замены и обмены не нужны в основном меню.
        base_rows.append([KeyboardButton(text="💞 Совместное время"), KeyboardButton(text="💬 Обсуждения")])

    if raw == Role.KOT.value:
        base_rows.append([KeyboardButton(text="💞 Совместное время"), KeyboardButton(text="💬 Обсуждения")])

    base_rows.append([KeyboardButton(text="🌤 Погода"), KeyboardButton(text="⚙️ Настройки")])
...
def schedule_type_keyboard(role: Role) -> InlineKeyboardMarkup:
    if role == Role.KOT:
        buttons = [
            [InlineKeyboardButton(text="💼 Рабочая смена / день", callback_data="schedule_type:work_day")],
            [InlineKeyboardButton(text="🌙 Ночная смена", callback_data="schedule_type:night_shift")],
            [InlineKeyboardButton(text="🕐 Занятость по времени", callback_data="schedule_type:custom_shift")],
