...
MENU_TEXTS = {
    "📝 Задачи", "📅 График", "⭐ Баллы", "💞 Совместное время",
    "🐾 Полезные лапки", "🧰 Полезная штука", "⚙️ Настройки", "🏡 Главное меню",
    "➕ Создать задачу", "📋 Мои задачи", "🏠 Все задачи", "✅ Выполненные", "💬 Обсуждения",
    "➕ Добавить мой день", "👀 Мой график", "👥 Общий график", "🔁 Замены", "🔎 Шансы для обмена",
    "🌤 Погода", "📊 Финансовая сводка", "📱 SMM-лапка", "🧠 Слово дня", "💬 Тема для вечера",
    "💡 Идея для вечера", "💌 Тёплое сообщение", "🧺 Мини-чеклист",
}


async def _maybe_interrupt_text_flow(message: Message, state: FSMContext, session: AsyncSession) -> bool:
    """Не даём кнопкам меню случайно становиться текстом задачи/комментарием."""
    if message.text not in MENU_TEXTS and message.text not in {"⬅️ Назад", "❌ Отмена"}:
        return False
    ...
    if message.text in {"❌ Отмена", "⬅️ Назад", "🏡 Главное меню"}:
        await message.answer("🐾 Ок, вернула котика в главное меню.", reply_markup=main_menu(user.role if user else None))
        return True

    await message.answer(
        "🐾 Похоже, ты нажал кнопку меню во время ввода. "
        "Я закрыла незавершённое действие, чтобы ничего не записалось случайно.\n"
        "Нажми нужную кнопку ещё раз.",
        reply_markup=main_menu(user.role if user else None),
    )
    return True
...
async def _notify_task_created(message: Message, session: AsyncSession, creator: User, task: Task) -> None:
    recipients: list[User] = []
    all_users = await get_all_users(session)

    if task.assignee_id is None:
        recipients = [u for u in all_users if u.telegram_id != creator.telegram_id]
    elif task.assignee_id != creator.id:
        recipients = [u for u in all_users if u.id == task.assignee_id]
    ...
    text = (
        ...
        f"{task_card(task)}\n\n"
        "Задача сразу появилась в общем домике 🏡"
    )
    for user in recipients:
        try:
            await message.bot.send_message(user.telegram_id, text, reply_markup=task_actions_keyboard(task.id))
        except Exception:
            pass


async def _notify_creator_for_review(
    *,
    ...
    """Отправляет задачу на проверку именно создателю.

    Если создатель сам нажал “Выполнено” по общей задаче, уведомляем второго котика информационно,
    но кнопку “Засчитать” всё равно разрешит только правило _can_approve.
    """
    recipients: list[User] = []
    if task.creator and task.creator.telegram_id != actor.telegram_id:
        recipients = [task.creator]
    else:
        recipients = await _other_users(session, actor.telegram_id)

    text = (
        f"🐾 {role_label(actor.role)} отметил задачу выполненной:\n\n"
        f"{task_card(task)}\n\n"
        "Задача ждёт проверки создателя. Если всё окей — можно нажать <b>Засчитать</b> ⭐"
    )

    for recipient in recipients:
        try:
            await callback.bot.send_message(recipient.telegram_id, text, reply_markup=task_review_keyboard(task.id))
        except Exception:
            pass

def _is_assignee(user: User, task: Task) -> bool:
    # None = задача для обоих, оба могут брать/выполнять.
...
def _can_approve(user: User, task: Task) -> bool:
    # Финальное правило: подтвердить/засчитать может только тот, кто создал задачу.
    return _is_creator(user, task)


def _can_close_or_rework(user: User, task: Task) -> bool:
    # Закрытие, возврат на доработку и изменение баллов — решения создателя задачи.
    return _is_creator(user, task)


@router.message(F.text == "📝 Задачи")
async def tasks_menu(message: Message, session: AsyncSession, state: FSMContext) -> None:
    user = await get_user_by_tg(session, message.from_user.id)
    if not user:
    ...
    await message.answer(
        "📝 <b>Раздел задач</b>\n\n"
        "Здесь живут бытовые миссии: можно создать задачу, посмотреть свои дела, общий список и обсуждения.\n"
        "Главное правило: баллы начисляются только после подтверждения второго котика ⭐",
        reply_markup=tasks_menu_keyboard(),
    )


@router.message(F.text == "🏡 Главное меню")
async def back_main_menu(message: Message, session: AsyncSession, state: FSMContext) -> None:
    user = await get_user_by_tg(session, message.from_user.id)
    await state.clear()
...
async def start_task_creation(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await message.answer(
        "📝 Напиши задачу, которую нужно выполнить\n\n"
        "Например: <i>купить продукты</i>, <i>помыть посуду</i>, <i>разобрать вещи</i>.\n"
...
async def task_title(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if await _maybe_interrupt_text_flow(message, state, session):
        return
    ...
    creator = await get_user_by_tg(session, message.from_user.id)
    if not creator:
        await message.answer("🐾 Сначала нажми /start и выбери роль.")
        await state.clear()
        return
    ...
    await message.answer(
        ...
        "Подсказка: самого себя как отдельного ответственного я не показываю. "
        "Если задача общая — выбирай 👥 Оба.",
        reply_markup=assignee_keyboard(creator.role),
    )


@router.callback_query(F.data == "task_cancel")
async def task_cancel(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    await state.clear()
    await callback.message.answer("🐾 Ок, действие отменено. Вернула тебя в задачи.", reply_markup=tasks_menu_keyboard())
...
async def task_points(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if value == "manual":
        ...
        await callback.message.answer("✍️ Введи количество звёздочек числом", reply_markup=cancel_back_keyboard())
        await callback.answer()
        return
...
async def task_manual_points(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if await _maybe_interrupt_text_flow(message, state, session):
        return
    try:
...
async def task_manual_deadline(message: Message, state: FSMContext, session: AsyncSession) -> None:
    if await _maybe_interrupt_text_flow(message, state, session):
        return
...
async def task_comment_choice(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    if value == "yes":
        ...
        await callback.message.answer("💬 Напиши комментарий", reply_markup=cancel_back_keyboard())
        await callback.answer()
        return
...
async def done_tasks(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer("✅ <b>Выполненные и ожидающие подтверждения задачи:</b>")
    for task in tasks[:15]:
        if task.status == TaskStatus.WAITING_APPROVAL:
            await message.answer(task_card(task), reply_markup=task_review_keyboard(task.id))
        else:
...
async def discussion_tasks(message: Message, session: AsyncSession) -> None:
    ...
    await message.answer("💬 <b>Задачи на обсуждении / подтверждении:</b>")
    for task in tasks[:15]:
        keyboard = task_review_keyboard(task.id) if task.status == TaskStatus.WAITING_APPROVAL else discussion_actions_keyboard(task.id)
        await message.answer(task_card(task), reply_markup=keyboard)
...
async def task_action(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action in {"progress", "done"} and not _is_assignee(user, task):
        await callback.answer("🐾 Эта задача назначена другому котику. Можно обсудить или посмотреть её, но выполнить должен ответственный.", show_alert=True)
        return
    ...
    if action == "done":
        if task.status == TaskStatus.COUNTED:
            await callback.answer("Эта задача уже засчитана ⭐", show_alert=True)
            return
        if task.status == TaskStatus.WAITING_APPROVAL:
            await callback.answer("🐾 Задача уже ждёт проверки создателя.", show_alert=True)
            return

        task = await mark_done_pending_approval(session, task)
        await callback.message.edit_text(
            f"{task_card(task)}\n\n"
            "⏳ Задача отправлена на проверку создателю. "
            "Звёздочки начислятся только после подтверждения ⭐"
        )
        await _notify_creator_for_review(callback=callback, session=session, actor=user, task=task)
    ...
    if action == "discuss":
        ...
        await callback.message.answer("💬 Напиши, что хочется уточнить по задаче.\nБез ссор — просто чтобы котики поняли друг друга 🐾", reply_markup=cancel_back_keyboard())
        await callback.answer()
        return

    if action == "cancel":
        if not _is_creator(user, task):
            await callback.answer("🐾 Закрыть задачу может только котик, который её создал. Можно нажать 💬 Обсудить.", show_alert=True)
            return
        task = await cancel_task(session, task, remove_points=True)
        await callback.message.edit_text(f"{task_card(task)}\n\n🕊 Задача закрыта создателем как неактуальная.")
        await _notify_others(
            message_or_callback=callback,
            session=session,
            actor_tg_id=callback.from_user.id,
...
async def task_review(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "count":
        if not _can_approve(user, task):
            await callback.answer("🐾 Засчитать задачу может только котик, который её создал.", show_alert=True)
            return
        task = await approve_task_and_sync_points(session, task, "Задача подтверждена вторым котиком")
        await callback.message.edit_text(f"{task_card(task)}\n\n🟢 Задача засчитана. Звёздочки синхронизированы ⭐")
        await _notify_others(
            message_or_callback=callback,
            session=session,
            actor_tg_id=callback.from_user.id,
    ...
    if action == "rework":
        if not _can_close_or_rework(user, task):
            await callback.answer("🐾 Вернуть на доработку может только котик, который создал задачу.", show_alert=True)
            return
        ...
        await _notify_others(
            ...
            text=(
                ...
                f"{task_card(task)}\n\n"
                "Баллы не начислялись / временно синхронизированы и вернутся после подтверждения ⭐"
            ),
            reply_markup=task_actions_keyboard(task.id),
        )
        await callback.answer()
...
async def discussion_reason(callback: CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    if not task or not user:
        await callback.answer("Не нашёл задачу 🐾", show_alert=True)
        return
    if reason in {"points", "rework", "cancel"} and not _can_close_or_rework(user, task):
        await callback.answer("🐾 Это действие доступно только котику, который создал задачу. Можно оставить комментарий.", show_alert=True)
        return
    await state.update_data(task_id=int(task_id_raw), discuss_reason=reason)
    await state.set_state(DiscussTask.text)
    if reason == "points":
        await callback.message.answer("⭐ Напиши новое количество баллов числом. Можно добавить комментарий после числа: <code>2, потому что задача лёгкая</code>", reply_markup=cancel_back_keyboard())
    else:
        await callback.message.answer("💬 Напиши комментарий к задаче", reply_markup=cancel_back_keyboard())
    await callback.answer()
...
async def discussion_text(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    if reason in {"rework", "points", "cancel"} and not _can_close_or_rework(author, task):
        await message.answer("🐾 Это действие может сделать только котик, который создал задачу. Я сохраню это как комментарий.")
        reason = "comment"
    ...
    await _notify_others(
        ...
        text=(
            ...
            f"{task_card(task)}\n\n"
            f"💬 Комментарий: “{text}”\n\n"
            "Это только обсуждение: задача не засчитана, пока второй котик не нажмёт <b>Засчитать</b> 🐾"
        ),
    ...
    await message.answer(
        ...
        "Задача не засчитана автоматически — всё честно ⭐",
        reply_markup=main_menu(author.role),
    )
...
async def discussion_action(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "count":
        if not _can_approve(user, task):
            await callback.answer("🐾 Засчитать задачу может только котик, который её создал.", show_alert=True)
            return
        task = await approve_task_and_sync_points(session, task, "Задача засчитана после обсуждения")
        await callback.message.edit_text(f"{task_card(task)}\n\n🟢 Обсуждение закрыто, задача засчитана, баллы синхронизированы ⭐")
        await _notify_others(
            message_or_callback=callback,
            session=session,
            actor_tg_id=callback.from_user.id,
            text=f"🟢 {role_label(user.role)} закрыл обсуждение и засчитал задачу:\n\n{task_card(task)}\n\nКотики договорились 💞",
        )
        await callback.answer()
        return
    ...
    if action == "close":
        if not _can_close_or_rework(user, task):
            await callback.answer("🐾 Закрыть обсуждение может только котик, который создал задачу.", show_alert=True)
            return
        # Важно: закрытие обсуждения не начисляет баллы. Возвращаем задачу в ожидание подтверждения.
        task = await set_task_status(session, task, TaskStatus.WAITING_APPROVAL)
        await callback.message.edit_text(f"{task_card(task)}\n\n🕊 Обсуждение закрыто. Задача снова ждёт подтверждения, без автозачёта 🐾")
        await _notify_others(
            message_or_callback=callback,
            session=session,
            actor_tg_id=callback.from_user.id,
            text=f"🕊 {role_label(user.role)} закрыл обсуждение без зачёта:\n\n{task_card(task)}\n\nЕсли всё окей — нажмите <b>Засчитать</b> ⭐",
    ...
    if action == "points":
        if not _can_close_or_rework(user, task):
            await callback.answer("🐾 Изменить баллы может только котик, который создал задачу.", show_alert=True)
            return
        task = await set_task_status(session, task, TaskStatus.POINTS_REVIEW)
        await callback.message.answer("⭐ Напиши новое количество баллов числом. Например: <code>2, потому что задача полегче</code>", reply_markup=cancel_back_keyboard())
    ...
    if action == "reply":
        await callback.message.answer("💬 Напиши ответ по задаче", reply_markup=cancel_back_keyboard())
        await state.update_data(task_id=task.id, discuss_reason="comment")
        await state.set_state(DiscussTask.text)
        await callback.answer()
        return
