...
def _can_approve(user: User, task: Task) -> bool:
    # Задачу с конкретным ответственным подтверждает второй котик, не ответственный.
    # Для “Оба” в MVP подтверждение может нажать любой второй участник; если нажмёт тот же — защита от дублей всё равно сработает.
    if task.assignee_id is None:
        return True
...
async def task_action(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "done":
        ...
        await callback.message.answer(
            "✅ Задача отправлена на подтверждение.\n"
            "Звёздочки начислятся после кнопки <b>Засчитать</b> у второго котика ⭐"
        )
        await _notify_others(
            message_or_callback=callback,
            session=session,
...
async def task_review(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "count":
        if not _can_approve(user, task):
            await callback.answer("🐾 Ответственный котик не может сам себе засчитать задачу. Нужна лапка второго котика.", show_alert=True)
            return
