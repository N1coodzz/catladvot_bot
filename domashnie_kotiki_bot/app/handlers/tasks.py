...
from app.models import CommentType, Role, TaskStatus
...
from app.services.users import get_partner, get_user_by_role, get_user_by_tg
from app.utils import fmt_dt, now_msk, parse_date_ru, role_label
...
async def task_manual_deadline(message: Message, state: FSMContext) -> None:
    ...
    # Если ввели только дату, datetime будет 00:00. Ставим конец дня.
...
async def task_action(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "done":
        ...
        partner = await get_partner(session, user)
        if partner:
            try:
...
async def discussion_text(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await add_comment(session, task, author, message.text.strip(), comment_type)
    await set_task_status(session, task, new_status)

    target_tg = None
    if task.assignee and task.assignee.telegram_id != author.telegram_id:
        target_tg = task.assignee.telegram_id
    elif task.creator.telegram_id != author.telegram_id:
        target_tg = task.creator.telegram_id
    ...
    await state.clear()
    await message.answer(
        "💬 Комментарий добавлен. Задача перешла в мягкое обсуждение 🐾",
        reply_markup=main_menu(),
    )
...
async def discussion_action(callback: CallbackQuery, session: AsyncSession, state: FSMContext) -> None:
    ...
    if action == "count" or action == "close":
        task = await count_task(session, task)
    ...
    if action == "points":
        ...
        await callback.message.answer("⭐ Напиши, какие баллы предлагаешь и почему")
