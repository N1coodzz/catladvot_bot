...
MENU_TEXTS = {
    ...
    "🧰 Полезная штука", "⚙️ Настройки", "🏡 Главное меню",
    ...
    "🌤 Погода", "💡 Идея для вечера", "💌 Тёплое сообщение", "🧺 Мини-чеклист",
}
...
async def _notify_task_created(
    ...
    """Сразу отправляет уведомление тем, кому задача важна.

    Если задача на партнёра — партнёру. Если задача на обоих — второму пользователю.
    Если пользователь ставит задачу самому себе — уведомление второму не отправляем.
    """
...
async def tasks_menu(message: Message, session: AsyncSession, state: FSMContext) -> None:
    ...
    await message.answer(
        "📝 <b>Раздел задач</b>\n\n"
        "Тут можно создать бытовую миссию, посмотреть свои дела, общий список и обсуждения.\n"
        "Если что-то пошло не так — всегда можно вернуться в главное меню 🐾",
...
async def start_task_creation(message: Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await message.answer(
        ...
        "Подсказка: если передумал — нажми ❌ Отмена.",
