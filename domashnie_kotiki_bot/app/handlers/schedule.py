...
from app.services.schedule_image import create_common_schedule_image
...
@router.message(F.text == "🖼 Красивый график")
async def common_schedule_image(message: Message, session: AsyncSession) -> None:
    user = await get_user_by_tg(session, message.from_user.id)
    await message.answer("🖼 Собираю красивую карточку графика. Сейчас котики всё разложат по полочкам 🐾")
    path = await create_common_schedule_image(session, days=7)
    if not path:
        await message.answer("🐾 Для красивого графика нужны оба пользователя: Кот и Котик.", reply_markup=schedule_menu_keyboard(user.role if user else None))
        return
    await message.answer_photo(
        FSInputFile(path),
        caption=(
            "🖼 <b>Красивый общий график</b>\n\n"
            "Здесь видно занятость, свободные окна и время вместе на ближайшие 7 дней 💞"
        ),
        reply_markup=schedule_menu_keyboard(user.role if user else None),
    )
